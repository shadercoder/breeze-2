...
{
	return static_cast<uint4>(m.properties.size());
}
...
// Sets the given (raw) values.
bool MaterialConfig::SetPropertyRaw(uint4 id, const void *values, size_t bytes)
{
	LEAN_ASSERT(id < m.properties.size());
...
// Gets the given number of (raw) values.
bool MaterialConfig::GetPropertyRaw(uint4 id, void *values, size_t bytes) const
{
	LEAN_ASSERT(id < m.properties.size());
