...
public:
	...
	BE_ENTITYSYSTEM_API static void AddControllers(EntityHandle entity, EntityController *const* controllers, uint4 count);
...
public:
	...
	LEAN_INLINE void AddController(lean::move_ptr<ActualType> controller) { AddControllerKeep(controller.peek()); controller.transfer(); }
	/// Adds the given controller.
	LEAN_INLINE void AddControllerKeep(EntityController *controller) { Entities::AddControllers(m_handle, &controller, 1); }
	/// Removes the given controller.
	...
	/// Adds the given controller.
	LEAN_INLINE void AddControllersKeep(EntityController *const* controllers, uint4 count) { Entities::AddControllers(m_handle, controllers, count); }
	/// Removes the given controller.
	...
	/// Gets the transformation.
	LEAN_INLINE const Transformation& GetTransformation() const { Entities::GetTransformation(m_handle); }
