...
class DevicePX : public lean::noncopyable_chain< beCore::TransitiveWrapper<physx::PxPhysics, DevicePX> >, public Device
{
private:
...
public:
	/// Constructor.
	BE_PHYSICS_PX_API DevicePX(const DeviceDesc &desc);
	/// Constructor.
	BE_PHYSICS_PX_API DevicePX(physx::PxPhysics *pPhysics,
		physx::PxDefaultCpuDispatcher *pCPUDispatcher = nullptr,
		physx::pxtask::CudaContextManager *pCudaContext = nullptr);
	/// Destructor.
	BE_PHYSICS_PX_API ~DevicePX();
...
template <> struct ToImplementationPX<Device> { typedef DevicePX Type; };
